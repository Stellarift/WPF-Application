...
namespace Game2048
{
    public partial class MainWindow : Window
    {
        ...
        private void InitializeBackgroundGrid() //очистка и заполнение серой подложки для плиток
        {
        ...
        {
            // предупреждение, чтобы игрок не сбросил рекорд случайно
            var result = MessageBox.Show(
                "Вы уверены, что хотите сбросить текущий рекорд до 0?",
                "Сброс рекорда",
            ...
            {
                // Обнуление переменной рекорда
                bestScore = 0;

                // Перезапись файла highscore.txt, отправляя туда 0
                SaveBestScore();

                // Обновляем циферки на экране
                UpdateScoreDisplay();

                // Запускаем новую игру, чтобы очистить поле
                NewGame();
            }
        }
        ...
        private Point GetCellCoordinates(int row, int col) //получение координат ячейки в Canvas 
        {
        ...
        private void AddRandomTile() // "рожалка" новой плитки на поле
        {
        ...
        private Border CreateTileVisual(int value) // Создание визуального представления плитки
        {
        ...
        private void AnimateMove(Border tile, int fromRow, int fromCol, int toRow, int toCol) // Анимация движения плитки
        {
            if (tile == null)
                return;
        ...
        private void AnimateSpawn(Border tile) // Анимация появления новой плитки
        {
        ...
        private void AnimateMerge(Border tile) // Анимация слияния плиток
        {
        ...
        private void MakeMove(Func<List<MoveInfo>> moveLogic) // Обработка хода игрока с анимацией
        {
            if (isAnimating)
                return;
        ...
        private void FinalizeMove(List<MoveInfo> moves) // Завершение хода после анимации, обновление сетки и проверка состояния игры
        {
            ClearAnimationCanvas();
            tileVisuals = new Border[GridSize, GridSize];
        ...
        private List<MoveInfo> ExecuteMoveLeft() // Логика движения плиток влево с учетом слияния и подсчетом очков
        {
        ...
        private List<MoveInfo> Move(MoveDirection direction) // Универсальная функция движения плиток в любом направлении с использованием поворота сетки
        {
            int rotations = 0;
        ...
        private Tuple<int, int> TransformCoordinates(int r, int c, int rotationCount) // Преобразование координат плитки в зависимости от количества поворотов сетки
        {
            int row = r, col = c;
        ...
        private void RotateMatrix90() // Поворот сетки на 90 градусов по часовой стрелке для упрощения логики движения плиток
        {
        ...
        private bool CheckGameOver() // Проверка, есть ли доступные ходы или пустые клетки на поле
        {
            for (int i = 0; i < GridSize; i++)
            {
                for (int j = 0; j < GridSize; j++)
        ...
        private bool CheckWin() // Проверка, достиг ли игрок плитки 2048
        {
            for (int i = 0; i < GridSize; i++)
                for (int j = 0; j < GridSize; j++)
        ...
        private Brush GetTileColor(int value) // Возвращает цвет плитки в зависимости от её значения
        {
            switch (value)
            {
        ...
        private Brush GetTextColor(int value) // Возвращает цвет текста плитки в зависимости от её значения
        {
            if (value <= 4) return new SolidColorBrush(Color.FromRgb(119, 110, 101));
            return new SolidColorBrush(Color.FromRgb(249, 246, 242));
        }

        private int GetFontSize(int value) // Возвращает размер шрифта текста плитки в зависимости от её значения
        {
        ...
        private void LoadBestScore() // Загрузка рекорда из файла при запуске игры
        {
            try
            {
        ...
        private void SaveBestScore() // Сохранение рекорда в файл после каждого хода, если он был побит
        {
            try { System.IO.File.WriteAllText("highscore.txt", bestScore.ToString()); }
            catch { }
        }

        private void Window_KeyDown(object sender, System.Windows.Input.KeyEventArgs e) // Обработка нажатий клавиш для управления игрой
        {
    ...
    public enum MoveDirection { LEFT, RIGHT, UP, DOWN } // Перечисление направлений движения плиток
